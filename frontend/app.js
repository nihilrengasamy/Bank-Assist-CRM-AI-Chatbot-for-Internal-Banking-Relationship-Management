...
function selectedCustomer() {
  return customers.find((customer) => customer.id === customerSelect.value) || customers[0];
}

function riskClass(risk) {
  return risk === "Elevated" ? "risk-elevated" : risk === "Medium" ? "risk-medium" : "";
}
...
async function loadCustomers() {
  ...
  customerSelect.innerHTML = customers
    .map((customer) => `<option value="${escapeHtml(customer.id)}">${escapeHtml(customer.name)} - ${escapeHtml(customer.segment)}</option>`)
    .join("");
  renderProfile(selectedCustomer());
  addMessage(
    "assistant",
    "Ready. Choose a customer and ask for a summary, risk view, next action plan, or relationship-manager follow-up.",
    "Assistant"
  );
}
